...
这里只测 requests 特有的部分：可选依赖、Session 配置、真流式。
"""
...
pytestmark = pytest.mark.skipif(not REQUESTS_AVAILABLE, reason="requests 未安装")
...
@pytest.fixture
def adapter() -> Iterator[RequestsAdapter]:
    instance = RequestsAdapter()
    try:
        yield instance
    finally:
        instance.close()
...
class TestOptionalDependency:
    def test_registered_when_installed(self):
        assert registry.ADAPTER_CLASSES.get("requests") is RequestsAdapter
    ...
    def test_constructing_without_requests_raises_with_install_hint(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr("ipclick.adapters.requests_adapter._requests", None)
        with pytest.raises(AdapterError, match=r"ipclick\[requests\]"):
            RequestsAdapter()

    def test_registry_hint_when_not_registered(self, monkeypatch: pytest.MonkeyPatch):
        """没装 requests 时，get_adapter 的报错应该是"缺依赖"而不是"尚未支持"——
        两者的处理方式完全不同：一个 pip install 能解决，一个不能。"""
        monkeypatch.delitem(registry.ADAPTER_CLASSES, "requests", raising=False)
        with pytest.raises(AdapterError, match=r"需要额外依赖.*ipclick\[requests\]"):
            registry.get_adapter("requests")


class TestSessionConfiguration:
    def test_does_not_trust_env_by_default(self, adapter: RequestsAdapter):
        """requests 默认 trust_env=True 会捡起环境里的 HTTP_PROXY——
        对一个"代发请求"的服务来说，这意味着出口 IP 悄悄变了。"""
        assert adapter.trust_env is False
        assert adapter._get_session(None, True).trust_env is False

    def test_explicit_proxy_is_set(self, adapter: RequestsAdapter):
        session = adapter._get_session("http://127.0.0.1:9", True)
        assert session.proxies == {"http": "http://127.0.0.1:9", "https": "http://127.0.0.1:9"}

    def test_separate_session_per_proxy_and_verify(self, adapter: RequestsAdapter):
    ...
    def test_verify_flag_applied(self, adapter: RequestsAdapter):
        assert adapter._get_session(None, False).verify is False

    def test_transport_retries_disabled(self, adapter: RequestsAdapter):
    ...
    def test_pool_size_from_settings(self):
        adapter = RequestsAdapter(AdapterSettings(max_connections=7, max_keepalive_connections=3))
        try:
    ...
    def test_timeout_is_split_into_connect_and_read(self, adapter: RequestsAdapter):
        """requests 传单值 timeout 时连接和读取共用一个值，
    ...
    def test_zero_timeout_falls_back_to_default(self, adapter: RequestsAdapter):
...
class TestStreaming:
    ...
    def test_header_arrives_before_body(self, adapter: RequestsAdapter, stream_server: str):
    ...
    def test_body_reassembles_exactly(self, adapter: RequestsAdapter, stream_server: str):
        _, body = self._collect(adapter.download_stream(f"{stream_server}/big", method="GET"))
        assert body == _CHUNK * _CHUNK_COUNT

    def test_arrives_in_multiple_chunks(self, adapter: RequestsAdapter, stream_server: str):
    ...
    def test_unreachable_server_yields_error_header(self, adapter: RequestsAdapter):
    ...
    def test_bad_method_yields_error_header(self, adapter: RequestsAdapter, stream_server: str):
        header = next(adapter.download_stream(f"{stream_server}/x", method="BREW"))
        assert isinstance(header, StreamHeader)
        assert header.status_code == -1
        assert "BREW" in (header.error or "")

    def test_early_close_does_not_leak(self, adapter: RequestsAdapter, stream_server: str):
