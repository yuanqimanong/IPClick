...
- **Web 管理端**：`ipclick run --web` 起一个带登录的界面——总览看板、实时请求流、
  「试一试」就地发请求看源码（支持粘贴 curl、可点名目标节点）、配置改完写回
  `ipclick.toml`、集群节点增删（保存即生效）与就地测连通性 / 鉴权、可选组件的
  装卸。左中右三栏布局 + 明暗主题切换，前端零构建链路
...
上面这些也可以**直接在 Web 端点按钮完成**（0.4 起）：`/components` 页列出全部五个
可选组件的两级安装状态，就地装 / 卸、下载浏览器本体，装完不用重启进程。
...
0.4 布局大改版：**左侧导航 + 主内容 + 右侧状态栏**的 CSS Grid，右上角有
**跟随系统 / 亮 / 暗** 三态主题切换（记在 `localStorage`，刷新不丢）。

| 页面 | 干什么 |
|---|---|
| **总览** `/` | 吞吐、成功率、在途与峰值、状态码分布、各适配器耗时与流量、集群拓扑、最近请求；右栏常驻服务端 / 安全 / 链路 / 限流 / 组件状态 |
| **请求流** `/trace` | **实时**看请求打进来。按状态 / 适配器 / URL 过滤，看目标站点排行与按天趋势 |
| **试一试** `/test` | 填一个网址（或直接**粘一条 curl**）就地发一次请求，看链路信息与返回的**源码**；可**点名**打到某个节点 |
| **组件** `/components` | 五个可选 extras 的两级安装状态，可以就地**安装 / 卸载 / 下载浏览器本体** |
| **配置** `/config` | 白名单内的行为配置可改，**写回 `ipclick.toml`**；可一键**生成凭据** |
| **节点** `/nodes` | 集群节点的增删改（**保存即生效**，不用重启）+ 就地**测试连接** |
...
填好 URL / 方法 / 请求头 / 请求体。认不出的参数会**明确列出来**——静默丢掉一个
`-F` 比不支持它更糟。

**指定目标节点**：开了服务端转发时，「试一试」多一个"目标节点"下拉，选中后跳过
负载均衡强制打到那一台。验证新加的机器配对没有，不用再反复点、靠轮询碰运气。

**测试连接**：`/nodes` 每行一个按钮，只验**连通性**与**集群内部鉴权**，不发业务
请求。失败时区分"连不上"（查进程和网络）和"鉴权不通过"（核对各节点 `.env` 里的
`IPCLICK_CLUSTER_SECRET`）——这两种的排查方向完全相反。底层是 0.4 新增的
`Ping` RPC；对着 0.3 的节点调会收到 `UNIMPLEMENTED`，那恰恰说明鉴权是通的，
页面会如实这么说，不会误报成鉴权失败。
...
- **不能改**：`[SECURITY]` 全部（令牌、TLS、SSRF 三个开关）、Web 自己的登录凭据、
  ...
  文件 / `.env` 后重启。
- **能装依赖**（0.4 放开，此前刻意不给）：`/components` 可以装 / 卸 IPClick 自己
  声明的那五个 extras，以及下载浏览器本体。包名走**白名单常量**（表单里传来的
  extra 必须能在清单里查到），命令以列表形式交给 `subprocess`（`shell=False`），
  并**绑定当前解释器**——不依赖 PATH 上的 `pip`，装到别的环境去比装不上更糟。
  `pip` 和 `uv pip` 两条路自动探测（uv 建的 venv 默认不装 pip），两者都没有时
  明确报错并给出手动命令。长任务（`camoufox fetch` 要下约 1 GB）跑在后台，
  页面轮询状态。**卸载只卸 Python 包**，浏览器本体不动——界面会把它的路径和
  体积摆出来，让人自己决定，从网页递归删一个 GB 级目录风险和收益不成比例。

这条线是刻意划的：这个服务能代任意 URL 发请求，一个能从网页关掉内网拦截、改掉
令牌的管理端，等于给自己装了个跳板。而调超时、加节点、装一个自己声明过的可选
依赖，这类操作改错了也只是性能或可用性问题，可逆、可见。
...
前端**没有构建链路**：没有模板引擎、框架、打包工具，也没有任何外部资源。
布局是纯 CSS Grid，交互是几十行原生 JS——0.3 时页面里一行 JS 都没有，0.4 加了
三件没有 JS 就做不好的事：手动切主题、装依赖的状态轮询、请求流的**局部刷新**
（`<meta refresh>` 每 3 秒重载整页，滚动位置丢失、正在填的过滤条件被冲掉、
页面白闪；现在只换那一块的 HTML，而且仍然由服务端渲染，不在 JS 里复制一份逻辑）。

CSP 里 `script-src` 用的是那两段内联脚本的 **sha256 哈希**，不是
`'unsafe-inline'`——脚本是源码里的常量，哈希能精确放行它们，同时让任何注入进来的
`<script>` 执行不了。
...
sqlite_path = "ipclick-trace.{port}.db"   # {port} = 运行时实际监听端口
...
`sqlite_path` 与 `[LOG].output` 的路径值支持 `{port}` 占位符，替换成**运行时实际
生效**的监听端口（`--port` 覆盖之后的那个，不是配置文件里的原始值）：
...
不这么做的话，同一目录用 `--port` 起的多个实例会**静默**写同一个库、抢同一个日志
文件：不报错、不提示，用户以为在看单个实例的数据，实际是多进程合并的结果。
（真撞上了也会打一条明确的告警，但那是兜底，不是解法。）

用占位符而不是在代码里"自动加端口后缀"是刻意的：新用户从模板拿到的默认值就带
`{port}`，天然按端口分离；已有部署里自己写死的路径**一个字符都不会被改**，否则
旧日志和 trace 数据看起来就像丢了。而且这条规则写在 toml 里是**看得见的**。

Web 端口也有了对应的命令行覆盖（gRPC 端口一直有 `--port`，Web 端此前没有）：
