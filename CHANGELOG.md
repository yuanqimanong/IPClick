...
- **`[SERVER].async_mode`（默认关，实验性）** —— 服务端换成 `grpc.aio` + 协程。
  ...
  | 端到端（并发 200） | 119.6 QPS | **208.9 QPS** | 1.7× |
  ...
  **为什么默认关**：① 自定义适配器只实现了同步 `download()`，异步模式下会被丢进
  ...
  Web 管理端与 `[CLUSTER].forward = "on"` 在异步模式下**都可用**：前者靠
  `send_from_thread` 跨线程投递回事件循环，后者由 `AsyncForwardingTaskService`
  组合复用同步转发器的路由决策。代价是转发出去的那一跳仍占一个线程——出站
  stub 还是同步的，收益集中在**入口自己执行**的那部分。
...
- **`[BROWSER].max_pages = 0` 按可用内存自动推导页面并发。**
  ...
  硬上限 16。**cgroup 限额与 `/proc/meminfo` 都读，取两者中较小的那个**——
  容器里后者报的是宿主机内存，只看它会在 512MB 的容器里开十几个页面然后被
  OOM killer 杀掉，而现象只是"容器莫名其妙重启"；只看前者则在没有 cgroup
  限额的裸机上拿不到数。取较小值两种环境都不会算过头。
...
- **客户端分发下的 QPS 分片。** `[CLUSTER].forward = "off"` 时调用方直连每一台
  ...
  **稳态速率和突发额度一起分。** 只分速率不分桶容量的话，每台仍攒着一整份未分片
  的令牌，N 台同时放出来就是 N 倍——10 台配 100 QPS，瞬时能放出 1000 个。
  这种漏法在稳态压测里完全看不出来（稳态是对的），只在**流量刚起来的那一下**
  暴露，而那恰恰是目标站点风控最容易触发的时刻，现象是"平时好好的，一重启或
  一扩容就被封"。节点数极多导致份额不足一个令牌时兜底到 1：容量为 0 的桶永远
  拿不到令牌，那是挂死不是限流。
...
- **限流配置写错时改为拒绝启动，不再静默变成「不限速」。**

  `per_host_qps = "abc"`、`-5`、`true` 这类值原先都会无声回落到 0.0，而 0.0 的
  语义恰好是**不限速**。也就是说配置写错的后果，是把限流彻底关掉，日志里一个字
  都没有：你以为配了 100 QPS，实际在满速锤目标站点，直到对面返 429 或封 IP 才
  发现，而配置文件看上去完全正常。

  限流是保护性开关，它只能 fail-closed 或吵闹地失败，不能安静地开闸。现在这类
  值一律抛 `ConfigError`（键不存在仍然走默认值——那是"没配"，不是"配错了"）。
  `[SERVER].async_mode` 同理：原先直接 `bool()` 原始值，于是 `async_mode = "false"`
  会**打开**实验性异步模式（`bool("false")` 是 True），现在能判读的字符串照收、
  含糊的报错。
...
- 新增 128 个测试（异步适配器、异步限流、集群分片与突发额度、浏览器内存预算、
  Web 桥接、异步服务端的 TLS、配置值守卫）。全量 1543 个用例。
