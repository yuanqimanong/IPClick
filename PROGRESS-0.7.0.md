...
### ✅ 阶段 5：浏览器并发上限
`resolve_max_pages()` in `adapters/browser_settings.py`
- [x] `max_pages = 0` → 按**可用内存**推导；`> 0` 时显式配置永远优先
- [x] 按引擎区分单页预算：camoufox 400MB、chromium 系 250MB（保守取值——
      少开一个只是慢一点，多开一个把机器推进 swap 就是"请求从几秒变几分钟、
      看起来像卡死"，且很难从现象联想到并发）
- [x] 留 1GB 系统余量，硬上限 16，CPU 核数只做封顶
- [x] **容器里优先读 cgroup 限额**，不看 `/proc/meminfo`——后者报的是宿主机内存，
      照它推导会在 512MB 容器里开十几个页面然后被 OOM killer 杀掉，
      而现象只是"容器莫名其妙重启"
- [x] 读不出内存 → 退回静态默认值，不自作聪明
- [x] 浏览器重建时重算（别的进程占多少内存是会变的），仅在值变化时打日志
- [x] 12 个测试

      写测试时踩了一个坑并记在用例里：第一版用 8192MB 做对照，结果两个引擎
      都撞到 16 的硬上限、断言恒等，测不出系数差异。

### ✅ 阶段 6：分布式限流
- [x] 异步令牌桶（阶段 4 已做）
- [x] `forward=off` 下**按存活节点自适应均分**（`cluster_share()` +
      `set_cluster_size()`），份额随健康探测变化，节点挂了幸存者立刻分到更多
- [x] 16 个测试，含"节点挂掉后份额要涨"这条——做不到的话四台挂到剩一台时，
      那一台仍只用 1/4 额度，集群吞吐掉到四分之一而没有任何地方报错
- [x] 配置注释写清**三种形态的语义差异**（单机 / forward=on / forward=off），
      并补上"多进程下每进程各算各的"这条同类陷阱

      ⚠️ **方案从"租约协调者"改成了"自适应均分"**（已与你确认）。理由：
      `forward = "on"` 时限流本来就精确，协调者是专为 `forward = "off"` 造的一套
      选主 + 租约 + 降级机制，而那个场景的用户切到 `forward = "on"` 就解决了。
      均分零协议改动、零新故障模式。代价是负载不均时浪费额度，已写进注释。

      租约协调者方案留在下面「待决策」里，等"额度浪费真的痛了"再做。

### ✅ 阶段 7：Web 端对齐
- [x] 五个新配置项全部进可编辑列表：`processes` / `max_concurrent_rpcs` /
      `max_concurrent_streams` / `compression` / `async_mode`
- [x] 每项的 hint 都带**实测数据**，而不是复述字段名。例如 `max_workers` 那条
      加了一句"调它没用——32/100/256 三档吞吐一样，要加吞吐看下面的工作进程数"，
      直接把人从错误的旋钮引开
- [x] 总览页新增「并发形态」一行，**多进程时明确提示链路记录只覆盖 0 号进程、
      总量约为实际的 1/N** —— 不说明的话人会去查链路配置、查磁盘、查 SQLite，
      而真相只是它本来就只统计了一个进程
- [x] 24 个测试，重点守"能保存进配置文件"而不只是"能显示"，
      以及写回时**注释不被吃掉**（写回是定点文本替换）

---

## 待你决策 / 遗留

1. **租约协调者**（阶段 6 的 B 方案，未做）
   - 现状：`forward=off` 按存活节点均分，负载不均时浪费额度
   - 若要精确且不浪费：按 id 选主，其余节点走已有 gRPC 通道批量申请额度，
     协调者持有权威令牌桶，失联时降级为静态均分
   - 代价：改 `task.proto` 加新 RPC（CI 有 protobuf 一致性检查）、选主逻辑、
     租约过期与记账、脑裂测试。工作量是均分方案的几倍
   - 触发条件：真的观察到额度浪费影响吞吐时再做

2. **DNS 那件事**（0.6.0 压测挖出来的，未改）
...
3. **trace 测试里那批整齐的 0.51s**（约 10 个用例）
